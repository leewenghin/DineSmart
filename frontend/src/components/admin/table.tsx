...
const Table = ({
  ...
  const [mainList, setMainList] = useState<(typeof TField)[]>([]);
  const [filteredList, setFilteredList] = useState<(typeof TField)[]>([]);
  const [variantGroupList, setVariantGroupList] = useState<(typeof TField)[]>(
    []
  );
  const [query, setQuery] = useState("");
  const [newVariantGroup, setNewVariantGroup] = useState<typeof TSubmitVariant>(
    {
      name: "",
      published: false,
    }
  );
  const [updateVariantGroup, setUpdateVariantGroup] = useState<
    (typeof TField)[]
  >([]);
  ...
  // Use map to extract the title field from each object
  ...
  const fetchSetVariantGroupIDList = async (
    ...
    try {
      const response = await fetch(`${setIDLink}${variantGroupID}/`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const fetchUpdateVariantGroupIDList = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch(`${getLink}${variantGroupID}/`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleCancel = (
    ...
    if (resetUpdateVariantGroup) {
      setUpdateVariantGroup(mainList);
    }
  ...
  useEffect(() => {
    fetchList(getLink, setMainList, setUpdateVariantGroup);
    page === "Variant" && fetchList(getVariantLink, setVariantGroupList);
  }, []);
  ...
  return (
    <div className="content px-10">
      <div className="flex justify-between items-center">
        {" "}
        <p className="title text-2xl font-bold my-2">{page}</p>
      </div>
      {/* <!-- Start block --> */}
                      ...
                      >
                        ...
                        {page == "Variant" && (
                          <td className="p-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            {/* If return object then print variantGroupList.name, if undefined leave blank */}
                            {variantGroupList.find(
                              (group) => group.id === item.title
                            )
                              ? variantGroupList.find(
                                  (group) => group.id === item.title
                                ).name
